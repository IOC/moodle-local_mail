YUI(M.yui.loader, {lang: M.local_mail_lang}).use('io-base', 'node', 'json-parse', 'panel', 'datatable-base', 'dd-plugin', 'moodle-form-dateselector', 'datatype-date', 'calendar-base', 'anim', function(Y) {
    ...
    var mail_create_edit_label_panel = function () {
        ...
        mail_edit_label_panel.addButton({
            ...
            action : function (e) {
                ...
                mail_edit_label_panel.set('left', 0);
    ...
    var mail_create_new_label_panel = function () {
        ...
        mail_new_label_panel.addButton({
            ...
            action : function (e) {
                ...
                mail_new_label_panel.set('left', 0);
    ...
    var mail_create_new_search_panel = function () {
        ...
        mail_new_search_panel.addButton({
            value  : M.util.get_string('search', 'local_mail'),
            section: Y.WidgetStdMod.FOOTER,
            action : function (e) {
                e.preventDefault();
                mail_new_search_panel.hide();
                mail_new_search_panel.set('left', 0);
                mail_do_search();
            }
        });
        mail_new_search_panel.addButton({
    ...
    var mail_hide_menu_search = function() {
        var menu = Y.one('#local_mail_form_search');
        if (menu) {
        ...
        if (M.form.dateselector.panel) {
            ...
            M.form.dateselector.panel.set('left', 0);
        }
    };
    ...
    var mail_cancel_search = function() {
    ...
    var mail_get_label_value = function(checkbox){
        var value;
        var classnames = checkbox.getAttribute('class').split(' ');
        var num;
        Y.each(classnames, function(classname){
            num = /mail_label_value_(\d+)/.exec(classname);
            if (num) {
        ...
        Y.each(M.local_mail.mail_labels, function (value, index) {
            if (Y.Array.indexOf(lblstoadd, index) != -1) {
                if (index != labelid) {
                    ...
                    if (!elem) {
                        elem = Y.Node.create('<span class="mail_label mail_label_' +
                                M.local_mail.mail_labels[index].color +
                                ' mail_label_' + index + '">' +
                                M.local_mail.mail_labels[index].name + '</span>');
    ...
    var mail_enable_all_buttons = function(bool) {
        ...
        if (Y.one('#mail_emptytrash') && Y.one('.mail_list .mail_adv_checkbox')) {
            mail_enable_button(Y.one('#mail_emptytrash'), true);
        }
    };
    ...
    var mail_show_hide_toolbar_buttons = function() {
        var toggle = Y.one('.mail_toggle_buttons');
        if (toggle) {
            var width = Y.one('body').get('offsetWidth');
            var node = Y.one('.mail_buttons');
            if (width < 990) {
                toggle.removeClass('mail_hidden');
                node.hide();
            } else {
                toggle.addClass('mail_hidden');
                node.show();
            }
        }
    };
    ...
    var mail_fixed_toolbar_buttons = function() {
        ...
        var posheader = (navbar ? navbar.get('offsetHeight') : 0);
    ...
    var mail_empty_trash_confirm_delete = function(e) {
        var message;
        message = M.util.get_string('emptytrashconfirm', 'local_mail');
        M.util.show_confirm_dialog(e,
                                    {
                                        'callbackargs' : ['emptytrash'],
                                        'callback' : mail_doaction,
                                        'message' : message,
                                        'continuelabel': M.util.get_string('emptytrash', 'local_mail')
                                    }
        );
    };
    ...
    Y.one("#region-main").delegate('click', function(e) {
        if (!this.hasClass('mail_button_disabled')) {
            mail_empty_trash_confirm_delete(e);
        }
    }, '#mail_emptytrash');
